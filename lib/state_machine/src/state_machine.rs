...
mod tests {
    ...
    #[test]
    fn event_pattern_precharge_complete_is_valid() {
        let event = Event::PrechargeComplete {
            from: Board::Telemetry,
            timestamp_ms: Timestamp(42),
            voltage_final_mv: Voltage(400_000),
        };
        ...
        match event {
            Event::PrechargeComplete {
                ...
                assert_eq!(from, Board::Telemetry);
                assert_eq!(timestamp_ms.0, 42);
                assert_eq!(voltage_final_mv.0, 400_000);
            }
            _ => panic!("Expected PrechargeComplete event"),
        }
    }

    #[test]
    fn event_pattern_levitation_ready_is_valid() {
        let event = Event::LevitationSystemsReady {
            ready: true,
            current_airgap_mm: Airgap(12),
            current_ma: Current(250),
        };
        ...
        match event {
            Event::LevitationSystemsReady {
                ...
                assert!(ready);
                assert_eq!(current_airgap_mm.0, 12);
                assert_eq!(current_ma.0, 250);
            }
            _ => panic!("Expected LevitationSystemsReady event"),
        }
    }

    #[test]
    fn event_pattern_discharge_complete_is_valid() {
        let event = Event::DischargeComplete {
            from: Board::Telemetry,
            timestamp_ms: Timestamp(5),
            voltage_final_mv: Voltage(0),
        };
        ...
        match event {
            Event::DischargeComplete {
                ...
                assert_eq!(from, Board::Telemetry);
                assert_eq!(timestamp_ms.0, 5);
                assert_eq!(voltage_final_mv.0, 0);
            }
            _ => panic!("Expected DischargeComplete event"),
        }
    }
}