...
pub async fn event_to_can() -> ! {
    ...
    let mut event_receiver = EVENT_BUS.receiver();
    ...
    loop {
        ...
        let can_message: Option<CanMessage> = match event {
            ...
            Event::Emergency { from, reason } => {
                Some(CanMessage::Emergency(from, reason))
            }
            ...
            // Calibration  
            Event::StartCalibrationCommand => {
                Some(CanMessage::StartCalibrationCommand)
            }
            Event::CalibrationComplete { from } => {
                Some(CanMessage::CalibrationComplete{ from })
            }

            // Electronics  
            Event::StartPrechargeCommand => {
                Some(CanMessage::StartPrechargeCommand)
            }
            Event::StartDischargeCommand => {
                Some(CanMessage::StartDischargeCommand)
            }
            Event::PrechargeStarted { from } => {
                Some(CanMessage::PrechargeStarted{ from })
            }
            Event::DischargeStarted { from } => {
                Some(CanMessage::DischargeStarted{ from })
            }
            Event::PrechargeComplete { from, voltage_cv } => {
                Some(CanMessage::PrechargeComplete {
                    from,
                    voltage: voltage_cv,
                })
            }
            Event::DischargeComplete { from, voltage_cv } => {
                Some(CanMessage::DischargeComplete {
                    from,
                    voltage: voltage_cv,
                })
            }
            Event::PrechargeFailed { from, reason } => {
                Some(CanMessage::PrechargeFailed {
                    from,
                    reason,
                })
            }
            ...
            Event::LevitationSystemsReady => {
                Some(CanMessage::LevitationSystemsReady)
            }
            Event::StartLevitationCommand => {
                Some(CanMessage::StartLevitationCommand)
            }
            Event::StopLevitationCommand => {
                Some(CanMessage::StopLevitationCommand)
            }
            Event::LevitationStarted { from } => {
                Some(CanMessage::LevitationStarted { from })
            }
            Event::LevitationStatus { from, airgap_μm, current_ma } => {
            ...
            Event::LevitationStopped { from } => {
                Some(CanMessage::LevitationStopped { from })
            }
            Event::LevitationFailed { from, reason } => {
                Some(CanMessage::LevitationFailed {
                    from,
                    reason,
                })
            }
            ...
            Event::UnclampBrakesCommand => {
                Some(CanMessage::UnclampBrakesCommand)
            }
            Event::ClampBrakesCommand => {
                Some(CanMessage::ClampBrakesCommand)
            }
            ...
            Event::BrakesClamped { from } => {
              Some(CanMessage::BrakesClamped{ from })
            }
            Event::BrakesUnclamped { from } => {
              Some(CanMessage::BrakesUnclamped{ from })
            }
            ...
            Event::DynamicsStatus { from, actuator_pressure_bar} => {
              Some(CanMessage::DynamicsStatus{ from, actuator_pressure_bar })
            }
            ...
            Event::StartPropulsionBrakingCommand => {
                Some(CanMessage::StartPropulsionBrakingCommand)
            }
            Event::PropulsionAccelerationStarted => {
                Some(CanMessage::PropulsionAccelerationStarted)
            }
            Event::PropulsionBrakingStarted => {
                Some(CanMessage::PropulsionBrakingStarted)
            }
            Event::PropulsionStatus { current_ma, velocity_kmh, temperature_c, voltage_cv } => {
            ...
            Event::PropulsionForce { force_n } => {
                Some(CanMessage::PropulsionForce { force_n })
            }
            Event::PropulsionFailed { from, reason } => {
                Some(CanMessage::PropulsionFailed {
                    from,
                    reason,
                })
            }
